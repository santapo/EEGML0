...
def get_regularizer(name, sigma=0.08):
    if name == "regularizer_1":
        return lambda x: 1 - torch.exp(-0.5*x**2/sigma**2)
    elif name == "regularizer_2":
        return lambda x: 1 - torch.sin(x/sigma)/x/sigma
    elif name == "regularizer_3":
        return lambda x: 1 - sigma**2 / (x**2+sigma**2)
    elif name == "regularizer_4":
        return lambda x: torch.exp(0.5*x**2/sigma)-torch.exp(-0.5*x**2/sigma) \
                                / (torch.exp(0.5*x**2/sigma)+torch.exp(-0.5*x**2/sigma))
    return None
